...
function Followers() {
  ...
  const getUserDetailsArr = async () => {
    ...
    try {
      const promises = userData?.Following?.map(async (username) => {
        return await appwriteService.getUserDetails(username);
      });
  ...
  useEffect(() => {
    ...
    userData?.Following.length !== 0 && getUserDetailsArr();
  }, [userData]);
  ...
  return userData?.Following.length !== 0 ? (
    ...
    >
      <div
        id="content"
        className="w-full sm:w-[75%] border-2 border-red-500 flex flex-col items-center justify-center border-b-0"
      >
        <div className="border-b-2 border-b-red-500 w-full pl-4 pb-4">
          <h1 className="text-red-500 text-3xl pt-4">Following</h1>
        </div>
